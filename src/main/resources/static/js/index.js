...
layui.config({
	base : "js/"
}).use(['bodyTab','form','element','layer','jquery','notice'],function(){
	var form = layui.form,
		layer = layui.layer,
	...
	tab = layui.bodyTab({
			...
			url : "/checklogin/getRole" //获取菜单json地址
		});
	...
	$("body").on("click","#unlock",function(){
		if($(this).siblings(".admin-header-lock-input").val() == ''){
			...
			$.ajax({
				...
				url: '/user/lockscreen',
				data:{
					"pwd":$(this).siblings(".admin-header-lock-input").val(),
					"name":$('#username').text(),
				},
				success:function (data) {
					if(data.code==200){
